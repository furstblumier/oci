...
# installing gobuster 
RUN wget http://ftp.br.debian.org/debian/pool/main/g/gobuster/gobuster_2.0.1-1_$(dpkg --print-architecture).deb &&\
         dpkg -i gobuster_2.0.1-1_$(dpkg --print-architecture).deb && rm gobuster_2.0.1-1_$(dpkg --print-architecture).deb
